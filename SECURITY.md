...
- **Blast radius if a guard above ever fails.** The only unattended write path is `knowledge-librarian.yaml`: `contents: write` / `pull-requests: write` only (no broader scope), commits touch only `.lokf/knowledge` and `.lokf/feedback.md` (never `git add -A`), it never pushes to the default branch or auto-merges, and merging the resulting PR is a human decision like any other change here. The workflow also triggers only on `schedule`/`workflow_dispatch`, never on an issue comment or other event an outside contributor could fire directly.
